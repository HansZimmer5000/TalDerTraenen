...
-define(KO, 			hole_wert_aus_config_mit_key(KO)).
...
prozess_loop(VZ, TZ, NS, KO, Name, Mi, Nachbarn, Q, LogDatei, VoteCount, VoteCfg) -> 
	...
	receive
		...
		{sendy, Y} ->
			MiNeu = calcGGT(Mi, Y, Nachbarn, KO, Name),
		...
		{voteYes, Name} ->
			{NewTerm, NewVoteCount, BriefFlag} = handleIncVote({false, false}, Q, VoteCount),			
			logge_status(io_lib:format("Vote von ~p erhalten.", [Name]), LogDatei),
			case BriefFlag of
				true ->
					briefTerm(KO, Name, Mi)					
			end,			
			prozess_loop(VZ, TZ, NS, KO, Name, Mi, Nachbarn, Q, LogDatei, NewVoteCount, {NewTerm, VoteRdyFlag})
...
calcGGT(Mi, Y, {LeftN, RightN}, KO, Name) -> 
    case Mi > Y of
        true -> 
			NewMi = ((Mi - 1) rem Y) + 1,			
			...
			LeftN !  {sendy, NewMi},
			RightN ! {sendy, NewMi},
			KoPid !  {briefmi, {Name, NewMi, vsutil:now2string(erlang:timestamp())}};
        false -> 
			NewMi = Mi
    end,
    NewMi.
...
kill(Name, NsPid) ->
	...
	unregister(Name),    
	ogge_status(io_lib:format("~p ist heruntergefahren.", [Name]), LogDatei),
	exit(kill).
